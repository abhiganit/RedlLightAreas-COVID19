...
for i = 1:6; % varying over RLAs
    for j = 1:4; % varying over R0
        ...
        yM = YM{i}{j};
        yML = YML{i}{j};
        cc0 = (sum(yM0(st+1:en,[CF]),2));
        ccl = (sum(yM(st+1:en,[CF]),2));
        ccr = (sum(yML(st+1:en,[CF]),2));
...
for i = 1:6
    ...
    for k = 1:4 % varying over R0
        ...
        for j = 1:3
            ...
            CumCases = [CumCases,sum(y{j}(st+1:en,[CF]),2)];
            CumCasesR = [CumCasesR,sum(y{j}(st+1:en,[CF2]),2)];
